...
func (s *InventoryServer) GetPart(
	...
	if req.GetUuid() == "" || req.GetUuid() == uuid.Nil.String() {
		return nil, status.Error(codes.InvalidArgument, "invalid part uuid")
	}
	...
	part := s.parts[uuid.MustParse(req.GetUuid())]
	if part.UUID == "" {
		return nil, status.Errorf(codes.NotFound, "part not found: %s", req.GetUuid())
	}

	return &inventoryv1.GetPartResponse{Part: &inventoryv1.Part{
		Uuid:          part.UUID,
		Name:          part.Name,
...
func (s *InventoryServer) ListParts(
	...
	if len(req.GetUuids()) > 0 {
		...
		for _, uuidStr := range req.GetUuids() {
			...
			if err != nil {
				return nil, status.Errorf(codes.InvalidArgument, "invalid uuid: %s", uuidStr)
			}

			part, ok := s.parts[parsed]
			if !ok {
				return nil, status.Errorf(codes.NotFound, "part not found: %s", uuidStr)
			}
